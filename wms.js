...
function _handleApiPost(e) {
  try {
    ...
    var allowedActions = [
      'getWarehouseAnalyticsData','calculatePrintTagList','savePrintTagLog',
    ...
    if (e && e.parameter && e.parameter.action) {
      ...
      try {
        ...
        var fnNames = [
          ...
          // ── ฟังก์ชันที่หน้าเว็บเรียกมานานแล้ว แต่ตกหล่นจาก whitelist ────────────
          // deployment เดิมใช้ logic คนละชุดจึงยังทำงานได้ พอรวมไฟล์แล้วต้องมีชื่อที่นี่
          // ไม่งั้นจะตอบ "Unknown action" ทันทีที่ deploy (Driver GPS / SO / Delivery Plan พัง)
          'getGpsActivityLog','saveGpsActivityRow','updateDriverActivityRows',
          'getSOLinesByCustomer','getPlannedShopIdsByDateRange','loGetOrderHistory',
        ];
...
// ════════════════════════════════════════════════════════════════
//  ผนวกจาก wms.js เดิม (รวมไฟล์ backend เหลือไฟล์เดียว 2026-08-26)
//  — sendEvalEmail (POST) + getProductSpec (GET) —
// ════════════════════════════════════════════════════════════════
