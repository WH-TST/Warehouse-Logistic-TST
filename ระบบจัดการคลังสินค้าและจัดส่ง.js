...
function _handleApiPost(e) {
  try {
    ...
    var fnMap = {
      // Analytics & Dashboard
      'getWarehouseAnalyticsData'    : getWarehouseAnalyticsData,
      'getSpaceAnalysisData'         : getSpaceAnalysisData,
      // Production Planning
      'getPlanByDate'                : getPlanByDate,
      'saveCalculationToNewSheet'    : saveCalculationToNewSheet,
      // Print Tag
      'calculatePrintTagList'        : calculatePrintTagList,
      'savePrintTagLog'              : savePrintTagLog,
      'getAllProductsForDropdown'     : getAllProductsForDropdown,
      // Inventory FG
      'autoLogInventoryDaily'        : autoLogInventoryDaily,
      'getFGCycleData'               : getFGCycleData,
      'updateFGCycleRow'             : updateFGCycleRow,
      'getFGProductList'             : getFGProductList,
      'runPolicyCheck'               : runPolicyCheck,
      'getStockInsight'              : getStockInsight,
      // Inventory SEMI
      'getSemiDashboardSummary'      : getSemiDashboardSummary,
      'saveSemiMovementAndGetMaster' : saveSemiMovementAndGetMaster,
      'getSemiCycleData'             : getSemiCycleData,
      'getSemiMasterList'            : getSemiMasterList,
      'getProductionVerifyData'      : getProductionVerifyData,
      // Safety Stock
      'saveSafetyStockData'          : saveSafetyStockData,
      // Logistic
      'getLogisticMasterData'        : getLogisticMasterData,
      'calcRouteDistance'            : calcRouteDistance,
      'saveLogisticPlan'             : saveLogisticPlan,
      'getLogisticPlans'             : getLogisticPlans,
      'getLogisticPlanById'          : getLogisticPlanById,
      'deleteLogisticPlan'           : deleteLogisticPlan,
      'getLogisticPlanSummary'       : getLogisticPlanSummary,
      // Pre-Shipment
      'getPreShipmentData'           : getPreShipmentData,
      'getPreShipmentProductList'    : getPreShipmentProductList,
      // Driver Activity
      'saveDriverActivityRow'        : saveDriverActivityRow,
      'saveDriverActivityRows'       : saveDriverActivityRows,
      'getDriverActivityLog'         : getDriverActivityLog,
      'deleteDriverActivityRow'      : deleteDriverActivityRow,
      'updateDriverActivityRows'     : updateDriverActivityRows,
      // GPS Activity
      'saveGpsActivityRow'           : saveGpsActivityRow,
      'saveGpsActivityRows'          : saveGpsActivityRows,
      'getGpsActivityLog'            : getGpsActivityLog,
      'deleteGpsActivityRow'         : deleteGpsActivityRow,
      // Driver Events
      'saveDriverEventRow'           : saveDriverEventRow,
      'saveDriverEventRows'          : saveDriverEventRows,
      'deleteDriverEventRow'         : deleteDriverEventRow,
      'updateDriverEventRow'         : updateDriverEventRow,
      // Staff Events
      'getStaffEventLog'             : getStaffEventLog,
      'saveStaffEventRow'            : saveStaffEventRow,
      'saveStaffEventRows'           : saveStaffEventRows,
      'updateStaffEventFixed'        : updateStaffEventFixed,
      'deleteStaffEventRow'          : deleteStaffEventRow,
      // Line / Driver Logs
      'getLineDriverLogs'            : getLineDriverLogs,
      // WH Activity
      'saveWHActivityRows'           : saveWHActivityRows,
      'getWHActivityLog'             : getWHActivityLog,
      // KPI
      'saveKPIResult'                : saveKPIResult,
      'getKpiWHLGData'               : getKpiWHLGData,
      'getKpiWHLGHistory'            : getKpiWHLGHistory,
      'saveKpiWHLG'                  : saveKpiWHLG,
    };
    ...
    var result = fnMap[action].apply(null, args);
    return ContentService
    ...
    if (e && e.parameter && e.parameter.action) {
      ...
      try {
        ...
        var fnNames = [
          ...
          'getLogisticMasterData','calcRouteDistance','saveLogisticPlan',
          'getLogisticPlans','getLogisticPlanById','deleteLogisticPlan',
          ...
          'getLineDriverLogs','saveWHActivityRows','getWHActivityLog',
...
/**
 * ✅ แก้ไข: ดึงยอดใช้จริงจาก Sheet 'Transection' 
 * เงื่อนไข: 
 * 1. Col D (Reference) = 'Production' 
 * 2. วันที่ต้องน้อยกว่าวันที่เลือก (สะสมถึงแค่เมื่อวาน)
 * 3. รวมยอดจาก Col E (CW quantity)
 */
/**
 * ✅ แก้ไข: ดึงยอดใช้จริงจาก Sheet 'Transection' 
 * เงื่อนไขช่วงเวลา: 
 * 1. เริ่มต้น: 13/01/2026 (รวมวันที่ 13 ด้วย)
 * 2. สิ้นสุด: ก่อนวันที่เลือก (ไม่รวมวันนี้)
 */
function getActualLinesExcludeDate(excludeDateStr) {
  try {
    const ss = SpreadsheetApp.openById(SPREADSHEET_ID);
    const sheet = ss.getSheetByName(TRANSECTION_SHEET); 
    
    if (!sheet || sheet.getLastRow() < 2) {
      Logger.log("⚠️ Transection Sheet ว่างเปล่า หรือชีตไม่เจอ (ชีตที่หา: '" + TRANSECTION_SHEET + "')");
      return {};
    }
    ...
    // ✅ Debug: ตรวจสอบ Header ของชีต Transection
    Logger.log("📋 Transection Header: " + JSON.stringify(data[0]));
    if (data.length > 1) Logger.log("📋 Row 1 sample: " + JSON.stringify(data[1]));

    // 1. วันที่เลือก (วันนี้) -> ตัดเวลาให้เป็น 00:00:00 เพื่อใช้เป็นเส้นชัย
    const excludeDate = new Date(excludeDateStr);
    excludeDate.setHours(0, 0, 0, 0); 
    
    // 2. วันเริ่มระบบ -> 13 ม.ค. 2026 เวลา 00:00:00
    const systemStartDate = new Date(2026, 0, 13); // (ปี, เดือน-1, วัน)
    systemStartDate.setHours(0, 0, 0, 0);

    Logger.log("🔍 Debug Actual Usage Range:");
    Logger.log("  - Start (>=): " + systemStartDate.toISOString().split('T')[0]); // 2026-01-13
    Logger.log("  - Stop  (<) : " + excludeDate.toISOString().split('T')[0]);     // วันที่เลือก (เช่น 2026-01-14)

    for (let i = 1; i < data.length; i++) {
      const rowDate = parseDateValue(data[i][0]);
      if (!rowDate) continue; 
      
      // ตัดเวลาของรายการทิ้ง ให้เหลือแค่วันที่ล้วนๆ
      rowDate.setHours(0, 0, 0, 0); 

      // 🚩 เงื่อนไขตรวจสอบช่วงเวลา (Time Range Check)
      // ต้องผ่านทั้ง 2 ด่าน:
      // ด่าน 1: วันที่รายการ >= 13/01/2026 (วันเริ่มระบบ)
      // ด่าน 2: วันที่รายการ <  วันที่เลือก (ไม่รวมวันนี้)
      
      const isAfterSystemStart = rowDate.getTime() >= systemStartDate.getTime();
      const isBeforeToday      = rowDate.getTime() < excludeDate.getTime();
      
      if (isAfterSystemStart && isBeforeToday) {
        
        const refType = String(data[i][3] || "").trim(); // Col D: Reference
        
        // ✅ แก้ไข: ใช้ toLowerCase() เพื่อป้องกัน case mismatch
        if (refType.toLowerCase() === "production") {
          const pid = String(data[i][1] || "").trim(); // Col B: Item
          const cwQty = Number(data[i][4]) || 0;       // Col E: CW quantity
          
          if (pid) result[pid] = (result[pid] || 0) + cwQty;
        }
      }
    }
...
//   "Customers"       → Col A=รหัสลูกค้า, B=ชื่อร้าน, C=ที่อยู่, D=เบอร์, E=เซลล์, F=ระยะทาง
...
const LOGI_SH_CUSTOMER  = 'Customers';
...
function getLogisticMasterData() {
  ...
  try {
    ...
    // ── 1. ทะเบียนรถ (TRUCK ID): Col A=ทะเบียน, B=ประเภท ───────────────────
    var trucks = [];
    ...
    if (truckSheet && truckSheet.getLastRow() >= 2) {
      ...
      for (var i = 1; i < td.length; i++) {
        ...
        if (plate) trucks.push({ plate: plate, type: type });
      }
    }

    // ── 2. คนขับ (Driver Name): Col A=รหัส, B=ชื่อ ──────────────────────────
    var drivers = [];
    ...
    if (driverSheet && driverSheet.getLastRow() >= 2) {
      ...
      for (var i = 1; i < dd.length; i++) {
        ...
        if (empName) drivers.push({ id: empId, name: empName });
      }
    }
    ...
    if (transSheet && transSheet.getLastRow() >= 2) {
      ...
      for (var i = 1; i < tt.length; i++) {
        ...
        if (transName) transports.push({ id: transId, name: transName });
      }
    }

    // ── 4. ร้านค้า (Customers): A=รหัส B=ชื่อ C=ที่อยู่ D=เบอร์ E=เซลล์ F=ระยะทาง G=ระยะทางฟรี ──
    var shops = [];
    ...
    if (custSheet && custSheet.getLastRow() >= 2) {
      ...
      for (var i = 1; i < cd.length; i++) {
        ...
        var address      = String(cd[i][2] || '').trim();
        var phone        = String(cd[i][3] || '').trim();
        var sale         = String(cd[i][4] || '').trim();
        var distance     = parseFloat(cd[i][5]) || 0;
        var freeDistance = parseFloat(cd[i][6]) || 0;  // Col G: ระยะทางฟรี (กม.)
        if (cid && cname) {
          shops.push({ id: cid, name: cname, address: address, phone: phone, sale: sale, distance: distance, freeDistance: freeDistance });
        }
      }
    }
...
// ╔══════════════════════════════════════════════════════════════════════╗
// ║  LINE OA + GEMINI VISION WEBHOOK                                     ║
// ║  บันทึกการปฏิบัติงานจากภาพถ่ายเอกสารลายมือในกลุ่ม LINE              ║
// ║                                                                      ║
// ║  Flow:                                                               ║
// ║    กลุ่ม LINE → LINE OA รับภาพ → GAS doPost() → Gemini Flash OCR    ║
// ║    → Parse JSON → บันทึกลง Sheet "LINE_Driver_Log" → ตอบกลับกลุ่ม  ║
// ║                                                                      ║
// ║  ข้อมูลที่อ่านจากเอกสารลายมือ:                                       ║
// ║    รหัสพนักงาน | วันที่ส่งมอบ | ทะเบียนรถ | จุด Stop              ║
// ║    เลขไมล์ต้นทาง | เลขไมล์ปลายทาง | เวลาออก | เวลาถึง             ║
// ║                                                                      ║
// ║  วิธีตั้งค่า (ทำครั้งเดียว):                                         ║
// ║    1. รันฟังก์ชัน setupLineWebhookKeys() หลังใส่ค่า Token/Key       ║
// ║    2. Deploy → New Deployment → Web App                              ║
// ║       Execute as: Me | Who has access: Anyone                       ║
// ║    3. คัดลอก Web App URL → วาง Webhook URL ใน LINE Developers       ║
// ║    4. เพิ่ม LINE OA เข้ากลุ่ม LINE ทีมจัดส่ง                        ║
// ╚══════════════════════════════════════════════════════════════════════╝

// ─── ชื่อ Sheet ที่เก็บข้อมูล (สร้างอัตโนมัติถ้ายังไม่มี) ───────────────
const LINE_DRIVER_LOG_SHEET = 'LINE_Driver_Log';

// ─── Header ของ Sheet LINE_Driver_Log ────────────────────────────────
const LINE_LOG_HEADERS = [
  'Timestamp',          // A: เวลาที่รับข้อมูล (dd/MM/yyyy HH:mm:ss)
  'รหัสพนักงาน',        // B
  'วันที่ส่งมอบ',        // C: dd/MM/yyyy
  'ทะเบียนรถ',           // D
  'จุด Stop',            // E: คั่นด้วย ,
  'ไมล์ต้นทาง',          // F
  'ไมล์ปลายทาง',         // G
  'เวลาออก',             // H: HH:MM
  'เวลาถึง',             // I: HH:MM
  'กม. (คำนวณ)',         // J: odomEnd - odomStart
  'ความมั่นใจ AI',       // K: high / medium / low
  'LINE Source',         // L: user / group / room
  'LINE User/Group ID',  // M
  'ข้อความดิบ (AI)'      // N: rawText ที่ Gemini อ่านได้
];

// ════════════════════════════════════════════════════════════════════════
// doPost — Entry Point (API call จาก GitHub Pages หรือ LINE Webhook)
// ════════════════════════════════════════════════════════════════════════
function doPost(e) {
  try {
    var rawBody  = e.postData ? e.postData.contents : '';
    // ── ตรวจสอบว่าเป็น API call (มี action key) หรือ LINE Webhook ──
    var payload = {};
    try { payload = JSON.parse(rawBody); } catch (pe) { payload = {}; }
    ...
    // ── ตรวจสอบ LINE Signature (ป้องกัน Fake Request) ─────────────
    if (secret) {
      var signature = e.parameter['x-line-signature'] ||
                      (e.headers && e.headers['X-Line-Signature']) || '';
      if (signature && !_lineVerifySignature(rawBody, secret, signature)) {
        Logger.log('[doPost] ❌ Signature ไม่ถูกต้อง — ปฏิเสธ Request');
        return ContentService
    ...
    _lineProcessWebhook(body);
  } catch (err) {
    Logger.log('[doPost] Error: ' + err.message);
  }
  // LINE Platform ต้องการ HTTP 200 OK ทันที
  return ContentService
    .createTextOutput(JSON.stringify({ status: 'ok' }))
    .setMimeType(ContentService.MimeType.JSON);
}

// ── ตรวจสอบ LINE Signature ด้วย HMAC-SHA256 + Channel Secret ────────
function _lineVerifySignature(body, secret, signature) {
  try {
    var mac     = Utilities.computeHmacSha256Signature(body, secret, Utilities.Charset.UTF_8);
    var macB64  = Utilities.base64Encode(mac);
    return macB64 === signature;
  } catch (err) {
    Logger.log('[_lineVerifySignature] ' + err.message);
    return false;
  }
}

// ════════════════════════════════════════════════════════════════════════
// ประมวลผล Webhook Events ทุกรายการ
// ════════════════════════════════════════════════════════════════════════
function _lineProcessWebhook(body) {
  if (!body.events || !body.events.length) return;

  var props     = PropertiesService.getScriptProperties();
  var lineToken = props.getProperty('LINE_TOKEN');
  var geminiKey = props.getProperty('GEMINI_KEY');

  if (!lineToken || !geminiKey) {
    Logger.log('[LINE Webhook] ⚠️ ยังไม่ได้ตั้งค่า LINE_TOKEN / GEMINI_KEY / LINE_SECRET — รัน setupLineWebhookKeys() ก่อน');
    return;
  }

  body.events.forEach(function (event) {
    // รับเฉพาะ message event ที่เป็นรูปภาพ
    if (event.type === 'message' && event.message && event.message.type === 'image') {
      _lineHandleImage(event, lineToken, geminiKey);
    }
  });
}
...
// ════════════════════════════════════════════════════════════════════════
function _lineHandleImage(event, lineToken, geminiKey) {
  var replyToken = event.replyToken;
  var messageId  = event.message.id;
  var sourceId   = _lineGetSourceId(event.source);

  try {
    // ── Step 1: ดาวน์โหลดภาพจาก LINE Content API ──────────────────
    var imageBlob = _lineDownloadImage(messageId, lineToken);
    if (!imageBlob) {
      _lineReply(replyToken, lineToken, '❌ ดาวน์โหลดภาพไม่ได้ กรุณาส่งใหม่อีกครั้ง');
      return;
    }

    // ── Step 2: ส่งภาพ → Gemini Vision OCR ────────────────────────
    var geminiText = _geminiOCR(imageBlob, geminiKey);
    if (!geminiText) {
      if (sourceId) _linePush(sourceId, lineToken, '❌ วิเคราะห์ภาพไม่สำเร็จ กรุณาตรวจสอบ GEMINI_KEY');
      return;
    }

    // ── Step 3: Parse ผลลัพธ์ JSON จาก Gemini ──────────────────────
    var data = _geminiParseJson(geminiText);

    // ── Step 4: บันทึกลง Google Sheets ─────────────────────────────
    var saved = _lineSaveToSheet(data, geminiText, event);

    // ── Step 5: Push ผลลัพธ์กลับเข้ากลุ่ม (Push ไม่มีหมดเวลา) ─────
    if (sourceId) {
      _linePush(sourceId, lineToken, _lineBuildSummary(data, saved));
    }

  } catch (err) {
    Logger.log('[_lineHandleImage] Error: ' + err.message);
    try { _lineReply(replyToken, lineToken, '⚠️ เกิดข้อผิดพลาด: ' + err.message); } catch(e2){}
  }
}
...
// ════════════════════════════════════════════════════════════════════════
function _lineDownloadImage(messageId, token) {
  try {
    ...
    if (res.getResponseCode() !== 200) {
      Logger.log('[_lineDownloadImage] HTTP ' + res.getResponseCode());
      return null;
    }
    return res.getBlob().setName('driver_log_' + messageId + '.jpg');
  } catch (err) {
    Logger.log('[_lineDownloadImage] ' + err.message);
    return null;
  }
}

// ════════════════════════════════════════════════════════════════════════
// Gemini 1.5 Flash — OCR เอกสารลายมือ → JSON
// ════════════════════════════════════════════════════════════════════════
function _geminiOCR(imageBlob, apiKey) {
  try {
    var b64  = Utilities.base64Encode(imageBlob.getBytes());
    var mime = imageBlob.getContentType() || 'image/jpeg';

    var prompt =
      'คุณคือระบบ OCR อัจฉริยะสำหรับเอกสารบันทึกการปฏิบัติงานขับรถส่งสินค้าที่เขียนด้วยลายมือภาษาไทย\n\n' +
      'อ่านภาพและสกัดข้อมูลต่อไปนี้อย่างระมัดระวัง:\n' +
      '1. empId        — รหัสพนักงาน (ตัวเลขหรือรหัส)\n' +
      '2. deliveryDate — วันที่ส่งมอบ ในรูปแบบ dd/MM/yyyy\n' +
      '3. licensePlate — ทะเบียนรถ เช่น 80-1234 หรือ กข 5678\n' +
      '4. stopPoints   — จุด Stop / ชื่อลูกค้า / สถานที่ปลายทาง (คั่นด้วยเครื่องหมายจุลภาค)\n' +
      '5. odomStart    — เลขไมล์ต้นทาง (ตัวเลข ไม่มีหน่วย)\n' +
      '6. odomEnd      — เลขไมล์ปลายทาง (ตัวเลข ไม่มีหน่วย)\n' +
      '7. timeDepart   — เวลาออก รูปแบบ HH:MM (24 ชั่วโมง)\n' +
      '8. timeArrive   — เวลาถึง รูปแบบ HH:MM (24 ชั่วโมง)\n\n' +
      ...
      '  "timeArrive": "HH:MM",\n' +
      '  "confidence": "high",\n' +
      '  "rawText": "ข้อความดิบทั้งหมดที่อ่านได้จากภาพ"\n' +
      '}\n\n' +
      'กฎ: ถ้าอ่านค่าใดไม่ออกให้ใส่ "" ห้ามเดา | confidence = high (>80%) / medium (50-80%) / low (<50%)';
    ...
    if (res.getResponseCode() !== 200) {
      Logger.log('[Gemini] HTTP ' + res.getResponseCode() + ' | ' + res.getContentText().substring(0, 400));
      return null;
    }
    ...
    Logger.log('[_geminiOCR] ' + err.message);
    return null;
  }
}

// ════════════════════════════════════════════════════════════════════════
// Parse JSON text จาก Gemini (รองรับ markdown code block)
// ════════════════════════════════════════════════════════════════════════
function _geminiParseJson(text) {
  try {
    var cleaned = text.replace(/```json\s*/gi, '').replace(/```\s*/g, '').trim();
    var match   = cleaned.match(/\{[\s\S]*\}/);
    if (!match) { Logger.log('[Parse] ไม่พบ JSON block | text: ' + text.substring(0,100)); return { rawText: text }; }
    return JSON.parse(match[0]);
  } catch (err) {
    Logger.log('[_geminiParseJson] ' + err.message);
    return { rawText: text };
  }
}

// ════════════════════════════════════════════════════════════════════════
// บันทึกข้อมูลลง Sheet "LINE_Driver_Log"
// ════════════════════════════════════════════════════════════════════════
function _lineSaveToSheet(data, rawText, event) {
  try {
    var ss    = SpreadsheetApp.openById(SPREADSHEET_ID);
    var sheet = ss.getSheetByName(LINE_DRIVER_LOG_SHEET);

    // สร้าง sheet ถ้ายังไม่มี
    if (!sheet) {
      sheet = ss.insertSheet(LINE_DRIVER_LOG_SHEET);
      sheet.appendRow(LINE_LOG_HEADERS);
      var hRange = sheet.getRange(1, 1, 1, LINE_LOG_HEADERS.length);
      hRange.setFontWeight('bold')
            .setBackground('#0f172a')
            .setFontColor('#ffffff')
            .setHorizontalAlignment('center');
      sheet.setFrozenRows(1);
      // ความกว้าง column: A B C D E F G H I J K L M N
      [150,100,110,120,220,100,100,80,80,90,110,80,140,300]
        .forEach(function(w, i) { sheet.setColumnWidth(i + 1, w); });
      Logger.log('[LINE] สร้าง Sheet "' + LINE_DRIVER_LOG_SHEET + '" ใหม่');
    }

    // คำนวณ km
    var kmCalc = '';
    var s  = parseFloat(data.odomStart);
    var en = parseFloat(data.odomEnd);
    if (!isNaN(s) && !isNaN(en) && en >= s) kmCalc = String(en - s);

    var now     = new Date();
    var src     = event.source || {};
    var srcType = src.type || 'unknown';
    ...
    // ไฮไลต์แถวตาม confidence
    var lastRow = sheet.getLastRow();
    var conf    = (data.confidence || '').toLowerCase();
    var rowBg   = conf === 'high' ? '#f0fdf4' : conf === 'medium' ? '#fffbeb' : conf === 'low' ? '#fff1f2' : '#f8fafc';
    sheet.getRange(lastRow, 1, 1, LINE_LOG_HEADERS.length).setBackground(rowBg);

    Logger.log('[LINE] ✅ บันทึกแถว ' + lastRow + ' | empId=' + data.empId + ' | conf=' + data.confidence);
    return { success: true, row: lastRow };

  } catch (err) {
    Logger.log('[_lineSaveToSheet] ' + err.message);
    return { success: false, message: err.message };
  }
}

// ════════════════════════════════════════════════════════════════════════
// สร้างข้อความสรุปตอบกลับกลุ่ม LINE
// ════════════════════════════════════════════════════════════════════════
function _lineBuildSummary(data, saved) {
  var conf  = (data.confidence || '').toLowerCase();
  var icon  = conf === 'high' ? '✅' : conf === 'medium' ? '⚠️' : '❓';
  var label = conf === 'high' ? 'มั่นใจสูง' : conf === 'medium' ? 'ควรตรวจสอบ' : 'อ่านไม่ชัด โปรดแก้ไข';

  var s  = parseFloat(data.odomStart);
  var en = parseFloat(data.odomEnd);
  var km = (!isNaN(s) && !isNaN(en) && en >= s) ? (en - s) + ' กม.' : '-';

  if (!saved.success) {
    return (
      icon + ' วิเคราะห์ภาพสำเร็จ (AI: ' + label + ')\n' +
      '⚠️ แต่บันทึกลง Sheets ไม่ได้: ' + (saved.message || 'ข้อผิดพลาดไม่ทราบสาเหตุ')
    );
  }
  ...
  return (
    '📋 บันทึกการปฏิบัติงาน\n' +
    '━━━━━━━━━━━━━━━━━━\n' +
    '👤 รหัสพนักงาน : ' + (data.empId        || '-') + '\n' +
    '📅 วันที่ส่งมอบ : ' + (data.deliveryDate || '-') + '\n' +
    '🚛 ทะเบียนรถ   : ' + (data.licensePlate || '-') + '\n' +
    '📍 จุด Stop    : ' + (data.stopPoints   || '-') + '\n' +
    '🔢 ไมล์        : ' + (data.odomStart || '-') + ' → ' + (data.odomEnd || '-') + ' (' + km + ')\n' +
    '⏰ ออก / ถึง   : ' + (data.timeDepart || '-') + ' / ' + (data.timeArrive || '-') + '\n' +
    '━━━━━━━━━━━━━━━━━━\n' +
    icon + ' AI: ' + label + '\n' +
    '✅ บันทึกใน Sheets แถวที่ ' + (saved.row || '?') + ' สำเร็จ'
  );
}

// ════════════════════════════════════════════════════════════════════════
// Helper: ดึง Source ID (groupId > roomId > userId)
// ════════════════════════════════════════════════════════════════════════
function _lineGetSourceId(source) {
  if (!source) return null;
  return source.groupId || source.roomId || source.userId || null;
}

// ════════════════════════════════════════════════════════════════════════
// Reply Message — ใช้ replyToken (ใช้ได้ครั้งเดียว ภายใน ~30 วินาที)
// ════════════════════════════════════════════════════════════════════════
function _lineReply(replyToken, token, text) {
  try {
  ...
  } catch (err) { Logger.log('[_lineReply] ' + err.message); }
}

// ════════════════════════════════════════════════════════════════════════
// Push Message — ใช้ groupId (ไม่มีหมดเวลา สำหรับส่งผลลัพธ์ OCR)
// ════════════════════════════════════════════════════════════════════════
function _linePush(to, token, text) {
  try {
    if (!to || !token) return;
  ...
  } catch (err) { Logger.log('[_linePush] ' + err.message); }
}

// ════════════════════════════════════════════════════════════════════════
// setupLineWebhookKeys — ตั้งค่า Token / Secret / Key ลง Script Properties
//
// ⚠️  วิธีใช้:
//   1. ใส่ค่าจริง 3 ค่าด้านล่าง (LINE_TOKEN, LINE_SECRET, GEMINI_KEY)
//   2. บันทึกไฟล์ใน Apps Script Editor (Ctrl+S)
//   3. Dropdown เลือกฟังก์ชัน "setupLineWebhookKeys" → กด ▶ Run
//   4. ตรวจ Execution Log ด้านล่าง → ต้องเห็น "✅ ตั้งค่าสำเร็จ"
//   5. ลบค่าจริงออก แล้วบันทึกใหม่ (เพื่อความปลอดภัย)
// ════════════════════════════════════════════════════════════════════════
function setupLineWebhookKeys() {
  var props = PropertiesService.getScriptProperties();

  // ⚠️ วิธีใช้: ใส่ค่าจริงตรงนี้ก่อนรัน แล้วลบทิ้งทันทีหลังรันสำเร็จ
  // ห้าม commit ค่าจริงลง source code เด็ดขาด
  props.setProperty('LINE_TOKEN',  'PASTE_YOUR_LINE_TOKEN_HERE');
  props.setProperty('LINE_SECRET', 'PASTE_YOUR_LINE_SECRET_HERE');
  props.setProperty('GEMINI_KEY',  'PASTE_YOUR_GEMINI_KEY_HERE');

  // ตรวจสอบว่าบันทึกสำเร็จ
  var t = props.getProperty('LINE_TOKEN');
  var s = props.getProperty('LINE_SECRET');
  var g = props.getProperty('GEMINI_KEY');

  Logger.log('✅ ตั้งค่า Script Properties สำเร็จ');
  Logger.log('LINE_TOKEN  = ' + (t ? t.substring(0, 15) + '... (' + t.length + ' ตัวอักษร)' : '❌ ว่าง'));
  Logger.log('LINE_SECRET = ' + (s ? s.substring(0, 8)  + '... (' + s.length + ' ตัวอักษร)' : '❌ ว่าง'));
  Logger.log('GEMINI_KEY  = ' + (g ? g.substring(0, 8)  + '... (' + g.length + ' ตัวอักษร)' : '❌ ว่าง'));
  Logger.log('─────────────────────────────────────────');
  Logger.log('✅ ขั้นตอนต่อไป: ลบค่าจริงออกจากโค้ด แล้วกด Save');
}

// ════════════════════════════════════════════════════════════════════════
// getLineDriverLogs — GAS function ให้ Frontend เรียกดูข้อมูล
// ดึงข้อมูลจาก Sheet LINE_Driver_Log เพื่อแสดงในแท็บการปฏิบัติงานจริง
// ════════════════════════════════════════════════════════════════════════
function getLineDriverLogs(filterDate) {
  try {
    var ss    = SpreadsheetApp.openById(SPREADSHEET_ID);
    var sheet = ss.getSheetByName(LINE_DRIVER_LOG_SHEET);
    ...
    for (var i = 1; i < rows.length; i++) {
      var r = rows[i];
      // A=Timestamp B=empId C=deliveryDate D=licensePlate E=stopPoints
      // F=odomStart G=odomEnd H=timeDepart I=timeArrive J=km
      // K=confidence L=srcType M=srcId N=rawText
      if (filterDate && String(r[2] || '').trim() !== filterDate) continue;

      data.push({
        timestamp:    String(r[0]  || ''),
        empId:        String(r[1]  || ''),
        deliveryDate: String(r[2]  || ''),
        licensePlate: String(r[3]  || ''),
        stopPoints:   String(r[4]  || ''),
        odomStart:    String(r[5]  || ''),
        odomEnd:      String(r[6]  || ''),
        timeDepart:   String(r[7]  || ''),
        timeArrive:   String(r[8]  || ''),
        km:           String(r[9]  || ''),
        confidence:   String(r[10] || ''),
        rawText:      String(r[13] || '')
      });
    }
...
function getKpiWHLGData(startDateStr, endDateStr) {
  try {
    ...
    // ── 4. KPI Log → Checker FG (%) + Checker SEMI (%) ────────────────
    // Col A(0)=บันทึกเมื่อ, B(1)=วันที่Cycle, C(2)=ประเภท, N(13)=KPI Checker(%)
    var kpiLogSheet = ss.getSheetByName('KPI Log');
    var kpiLogMap   = {}; // key yyyy-MM-dd → { FG: %, SEMI: % }  (last row wins)
    if (kpiLogSheet && kpiLogSheet.getLastRow() >= 2) {
      ...
      for (var p = 1; p < kpiLogData.length; p++) {
        ...
        var kPct  = parseFloat(String(kpiLogData[p][13]).replace(/,/g,'')); // Col N = KPI Checker(%)
        if (isNaN(kPct)) continue;
        if (!kpiLogMap[kKey]) kpiLogMap[kKey] = {};
        kpiLogMap[kKey][kType] = kPct; // last row for same date/type wins
      }
    }
    ...
    while (cur <= end) {
      ...
      // คำนวณ Inv Control (%)
      var totalLines     = totalLinesMap[key] !== undefined ? totalLinesMap[key] : null;
      var adjustedLines  = adjustedLinesMap[key] || 0;
      var invControlPct  = (totalLines !== null && totalLines > 0)
          ? Math.round(((totalLines - adjustedLines) / totalLines) * 1000) / 10
          : null;
      ...
      if (wha && wha.wpuList.length > 0 && wpuMedian && mpiMedian) {
        ...
        for (var t = 0; t < wha.wpuList.length; t++) {
          ...
          if (mpi_t > 0) {
            var eff_t = ((wpu_t / wpuMedian) * 0.7 + (mpiMedian / mpi_t) * 0.3) * 100;
            effList.push(eff_t);
          }
        }
        if (effList.length > 0) {
          // Step 1: เฉลี่ย efficiency แต่ละ trip
          effStep1 = effList.reduce(function(a,b){return a+b;},0) / effList.length;
          // Penalty: โหลดสินค้าผิด −25/ครั้ง, เคลมสินค้า −10/ครั้ง
          efficiency = Math.round((effStep1 - wrongCount * 25 - claimCount * 10) * 10) / 10;
        }
      ...
      // Checker FG/SEMI จาก KPI Log Col N
      var kpiLog        = kpiLogMap[key] || {};
      var checkerFGPct  = (kpiLog['FG']   !== undefined) ? Math.round(kpiLog['FG']   * 10) / 10 : null;
      var checkerSEMIPct = (kpiLog['SEMI'] !== undefined) ? Math.round(kpiLog['SEMI'] * 10) / 10 : null;
      ...
      result.push({
        ...
        effStep1:         effStep1   !== null ? Math.round(effStep1   * 10)  / 10  : null,
        wrongCount:       wrongCount,
        claimCount:       claimCount,
        wpuActual:        wpuActual  !== null ? Math.round(wpuActual  * 100) / 100 : null,
        mpiActual:        mpiActual  !== null ? Math.round(mpiActual  * 100) / 100 : null,
        wpuMedian:        wpuMedian  !== null ? Math.round(wpuMedian  * 100) / 100 : null,
        mpiMedian:        mpiMedian  !== null ? Math.round(mpiMedian  * 100) / 100 : null,
        loadCount:        wha ? wha.wpuList.length : 0,
        planMinutes:      planMinutes,
        invControlPct:    invControlPct,
        checkerFGPct:     checkerFGPct,
        checkerSEMIPct:   checkerSEMIPct,
        damagePct:        damagePct,
        dmgLoadCount:     dmgLoadCount,
        dmgStoreCount:    dmgStoreCount,
      });
...
function saveKpiWHLG(row) {
  try {
    ...
    var headers = [
      'วันที่',
      'ประสิทธิภาพโหลด (%)',
      'น้ำหนัก/หน่วย จริง (kg)',
      'นาที/รายการ จริง',
      'น้ำหนัก/หน่วย Median',
      'นาที/รายการ Median',
      'จำนวนรายการโหลด',
      'แผนผลิต (นาที)',
      'BD ไม่มีพื้นที่ (นาที)',
      'ไม่มีพื้นที่ (%)',
      'Inv Control (%)',
      'Checker FG (%)',
      'Checker SEMI (%)',
    ...
    var newRow = [
      ...
      _v(row.bdMinutes),
      _v(row.noSpacePct),
      _v(row.invControlPct),
      _v(row.checkerFGPct),
      _v(row.checkerSEMIPct),
      _v(row.damagePct),
      _v(row.dmgLoadCount),
      _v(row.dmgStoreCount),
      _v(row.safety),
      nowStr,
    ];

    // ตรวจสอบว่ามีข้อมูลวันนี้อยู่แล้วหรือไม่ (อัปเดตถ้ามี)
    var lastRow  = sheet.getLastRow();
    var updated  = false;
    if (lastRow >= 2) {
      var existing = sheet.getRange(2, 1, lastRow - 1, 1).getValues();
      for (var i = 0; i < existing.length; i++) {
        var cellDate = existing[i][0] instanceof Date
          ? Utilities.formatDate(existing[i][0], 'GMT+7', 'dd/MM/yyyy')
    ...
    var targetRow = updated
      ? (function(){ var d=sheet.getRange(2,1,sheet.getLastRow()-1,1).getValues(); for(var x=0;x<d.length;x++){var c=d[x][0] instanceof Date?Utilities.formatDate(d[x][0],'GMT+7','dd/MM/yyyy'):String(d[x][0]||'').substring(0,10); if(c===row.date)return x+2;} return sheet.getLastRow(); })()
      : sheet.getLastRow();
...
function getKpiWHLGHistory(startDateStr, endDateStr) {
  try {
    ...
    for (var i = 1; i < data.length; i++) {
      ...
      var _pf = function(v) { return v !== '' && v !== null ? parseFloat(v) : null; };
      var _pi = function(v) { return v !== '' && v !== null ? parseInt(v)   : null; };
      // Columns (ปัจจุบัน):
      //  0=วันที่  1=ประสิทธิภาพ  2=wpuActual  3=mpiActual  4=wpuMedian  5=mpiMedian
      //  6=loadCount  7=แผนผลิต(นาที)  8=BD(นาที)  9=ไม่มีพื้นที่(%)
      // 10=invCtrl  11=checkerFG  12=checkerSEMI
      // 13=Damage(%)  14=Damage-โหลด  15=Damage-เก็บ  16=Safety  17=บันทึกเมื่อ
      result.push({
        date:            ddmmyyyy,
        efficiency:      _pf(data[i][1]),
        wpuActual:       _pf(data[i][2]),
        mpiActual:       _pf(data[i][3]),
        wpuMedian:       _pf(data[i][4]),
        mpiMedian:       _pf(data[i][5]),
        loadCount:       _pi(data[i][6]),
        planMinutes:     _pi(data[i][7]),
        bdMinutes:       _pi(data[i][8]),
        noSpacePct:      _pf(data[i][9]),
        invControlPct:   _pf(data[i][10]),
        checkerFGPct:    _pf(data[i][11]),
        checkerSEMIPct:  _pf(data[i][12]),
        damagePct:       _pf(data[i][13]),
        dmgLoadCount:    _pi(data[i][14]),
        dmgStoreCount:   _pi(data[i][15]),
        safety:          _pi(data[i][16]),
      });
    }
    return { success: true, data: result };
  } catch(e) {
