    ...
    function makeBuilder(table) {
        ...
        async function execute() {
            ...
            if (op === 'upsert') {
                ...
                // ⚠️ ต้องใช้ onConflict ที่ผู้เรียกระบุมาเอง (opArgs[1]) ถ้ามี — บางตารางใช้
                // unique constraint อื่นที่ไม่ใช่ PK จริง (เช่น truck_inspections ใช้
                // inspect_date+truck_plate+driver_id กันตรวจซ้ำ ไม่ใช่ id) ถ้า override เป็น PK
                // เฉยๆ จะจับคู่แถวเดิมผิด แล้วชน unique constraint จริงตอนเขียน (23505)
                var patchInput = opArgs[0] || {};
                var patchArray = Array.isArray(patchInput) ? patchInput : [patchInput];
                var opts2 = opArgs[1] || {};
                var pks2 = opts2.onConflict
                    ? opts2.onConflict.split(',').map(function(s) { return s.trim(); })
                    : pkCols(table);
                function _keyOf2(row) { return pks2.map(function(c) { return row[c]; }).join(''); }
                var found2;
                if (filterChain.length && patchArray.length === 1) {
                    ...
                    // composite key — ตารางกลุ่มนี้ไม่ใหญ่ ดึงทั้งตารางมาจับคู่ได้ไม่กระทบ perf
                    found2 = await mergedSelect(table, []);
                }
                var byKey2 = {};
                found2.rows.forEach(function(r) { byKey2[_keyOf2(r)] = r; });
                var fullRows2 = patchArray.map(function(patch) {
                    var existing = byKey2[_keyOf2(patch)];
